...
Why we need to initialize `VanillaTitl` inside the `useEffect` hook?

- `titlRef.current` outside the `useEffect` hook will at first be undefined
- `useEffect` is called after the component is rendered (mounted) and by this
  time it will already reference to some valid node
