...
- `useEffect` dependencies array does a shallow comparison between values
- putting array or object will always trigger use effect callback call
- primitive object properties inside object will not trigger callback calls,
  e.g.

```
  const object = { test: "test value"};
